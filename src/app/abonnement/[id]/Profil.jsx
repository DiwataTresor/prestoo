...
const Profil = ({ profil,data, callback }) => {
  ...
  return (
    ...
    <div className='flex justify-center items-center'>
      <div className='border rounded-md px-4 w-[40%] h-fit py-5 shadow-md border-gray-100 flex flex-col justify-center items-center'>
        ...
        <div className='flex flex-col gap-6'>
          <p className='bg-gray-100 px-2 py-2'>Abonnement actuel : <Chip>{typeAbonnement[profil?.typeabonnement]}</Chip></p>
          <p className='bg-gray-100 px-2 py-2'>Abonnement demandé : <Chip color='success' className='text-white'>{typeAbonnement[data?.typeAbonnement]} </Chip> Pour { data?.nbreAnnee +"Années(s)"}</p>
        ...
        <div className='flex gap-5 mt-5 border-t pt-4 w-full justify-center items-center'>
          <Button color='primary' onPress={()=>{
            Modal.confirm({
              title:"Abonnement",
              content:"Validez-vous vraiment cette demande ?",
              okText:"Oui",
              cancelText:"Annuler",
              onOk:() => {
                setSpinning(true);
                postData("abonnementDemandeValidation",{profilId:profil?.id})
                .then((r) => {
                  if(r.success)
                  {
                    // let newProfil=JSON.parse(Cookies.get("profil"));
                    // newProfil.typeabonnement=r.typeAbonnement;
                    // Cookies.set("profil",newProfil);
                    api.success({message:"Reabonnement",description:"Votre reabonnement est bien activé"});
                    router.push("/");

                  }else{
                    api.success({message:"Reabonnement",description:"Echec d'execution de la requete"});
                  }
                }).catch(r=>{
                  api.success({message:"Reabonnement",description:"Une erreur s'est produite dans le système"});
                }).finally(() =>{
                  setSpinning(false);
                })
              },
