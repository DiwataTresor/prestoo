...
const page=()=>{
    ...
    const colones = [
        ...
        {name: "DATE CERTIFICATION", uid: "DateReponse" },
      ...
      const INITIAL_VISIBLE_COLUMNS = ["DateReponse", "nom","villeDetail", "telephone","whatsapp", "actions"];
        ...
        switch(colone)
        {
            ...
            case "actions":
                return (<div className="flex flex-row gap-3 justify-center items-center">
                    ...
                    <Link href={`/profil/comptescertifies/${ligne?.idCertificationCrypte}`} target="_self"><Button isIconOnly size="md" color={"success"} variant="light"><Eye /></Button></Link>
                    <Button onPress={()=>{
                        ModalAnt.confirm({
                            title:"Suppression",
                            content:`Voulez-vous vraiment annuler cette certification ?`,
                            okText:"Supprimer",
                            cancelText:"Annuler",
                            onOk:()=>{
    ...
    const openNotificationError = () => {
      ...
      const datas=()=>{
        getData("comptescertifies").then(r=>{
            setData(r.data);
         }).catch(err=>{
  ...
  return(
    <>
    {contextHolder}
    <Section showRootLink={true} titre={"Comptes certifiés"} cl="text-xl" titreIcone={<BadgeCheck color="white" />}>
